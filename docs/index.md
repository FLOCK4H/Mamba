...
<div class="home-hero">
  ...
  <h1>Find the pool. Build the tx. Send it when you mean it.</h1>
  <p>Mamba is a Solana Rust kit for sniffing markets, streaming websockets, building swaps and launches, and running wallet ops. Drive it from the Local API, the terminal app, or an MCP agent. Signing stays local. You get receipts you can verify, not vibes.</p>
...
<div class="home-grid">
  <div class="surface-card">
    ...
    <p>Swaps, creates, pool ops, transfers, and wallet cleaning. Build first. Execute only when you flip it. Signing happens inside Mamba, not in your client.</p>
  </div>
  <div class="surface-card">
    <span class="surface-label">Degen surface</span>
    <strong>CLI and TUI</strong>
    <p>One terminal app for create, swap, dashboard, wallet ops, and the cleaner. Deterministic snapshots, so you can replay the story when it matters.</p>
  </div>
  <div class="surface-card">
    <span class="surface-label">Agent surface</span>
    <strong>MCP server</strong>
    <p>`mamba_mcp` mirrors the authenticated API so your agent can queue trades, mints, transfers, cleanup, and pool actions without ever seeing your keys.</p>
  </div>
  <div class="surface-card">
...
- Devnet rehearsal is expected. Mainnet sends stay manual unless you explicitly unlock them.
